...
import { Search, Shield, BarChart3, Zap, Bell, Brain, Monitor, Network, Eye, Activity, AlertTriangle, Lock, Menu, X, Twitter, Linkedin, Github, Check } from "lucide-react";
...
const LandingPage = () => {
  ...
  const pricingPlans = [
    {
      name: "Free Tier",
      description: "Prove the value. See blocking in action on a single endpoint.",
      monthlyPrice: "0",
      yearlyPrice: "0",
      buttonText: "Start for Free",
      features: [
        "Monitor 1 single device",
        "Full AI Threat Blocking enabled",
    ...
    {
      name: "Pro Network",
      description: "Scale your protection to cover the entire network.",
      monthlyPrice: "50",
      yearlyPrice: "40",
      popular: true,
      buttonText: "Go Pro",
    ...
    {
      name: "Enterprise",
      description: "Full-scale security for complex organizations.",
      monthlyPrice: "Custom",
      yearlyPrice: "Custom",
  ...
  return (
    <div ref={containerRef}>
              ...
              <a
                ...
                onClick={(e) => {
                  ...
                  if (item.name === 'Features') {
                    // Update URL to include #features
      ...
      {/* Final Phase Content: Use Cases, Pricing, Footer */}
      {phase === 'final' && (
        <section
          id="use-cases"
          ref={useCasesRef}
        ...
        >
          ...
          <div className="relative min-h-screen flex flex-col">
                ...
                <div className="flex gap-0 overflow-x-auto">
                  {useCaseTabs.map((tab, index) => (
                    <button
                      ...
                      className={`px-6 py-4 text-xs tracking-widest font-medium whitespace-nowrap transition-all duration-300 border-b-2 ${activeTab === index
                            ...
                            >
                              <div className="flex items-start justify-between gap-4">
                                <div className="flex items-start gap-3">
                                  <AlertTriangle className={`w-4 h-4 mt-0.5 ${alert.severity === 'critical' ? 'text-red-400' :
                                ...
                                <span className={`text-xs px-2 py-1 rounded ${alert.severity === 'critical' ? 'bg-red-500/20 text-red-300' :
                        ...
                        <div className="space-y-0">
                          {(useCaseTabs[activeTab].visual.data as Array<{ time: string, event: string, status: string }>).map((item, idx) => (
                            <div key={idx} className="flex items-start gap-4 pb-4 relative">
                              ...
                              <div className={`w-4 h-4 rounded-full flex-shrink-0 mt-1 ${item.status === 'resolved' ? 'bg-green-500' :
            ...
            <div
              id="pricing"
              className={`max-w-7xl mx-auto px-8 md:px-16 lg:px-24 py-16 md:py-24 transition-all duration-1000 ${useCasesVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}
            >
              <div className="text-center mb-16">
                ...
                <p className="text-gray-400 text-lg mb-8 max-w-2xl mx-auto" style={{ fontFamily: 'Nunito, sans-serif' }}>
                  Start securing your network today. No hidden fees, cancel anytime.
                </p>

                {/* Billing Toggle */}
                <div className="flex items-center justify-center gap-4 mb-8">
                  <span className={`text-sm ${!isAnnual ? 'text-white' : 'text-gray-500'}`} style={{ fontFamily: 'Nunito, sans-serif' }}>Monthly</span>
                  <button
                    onClick={() => setIsAnnual(!isAnnual)}
                    className="w-14 h-7 rounded-full bg-slate-800 border border-slate-700 relative transition-all"
                  >
                    <div
                      className={`absolute top-1 bottom-1 w-5 rounded-full bg-[#4a9eda] transition-all duration-300 shadow-lg ${isAnnual ? 'right-1' : 'left-1'}`}
                    />
                  </button>
                  <span className={`text-sm ${isAnnual ? 'text-white' : 'text-gray-500'}`} style={{ fontFamily: 'Nunito, sans-serif' }}>
                    Yearly <span className="text-[#4a9eda] text-xs font-bold ml-1">-20%</span>
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {pricingPlans.map((plan, index) => (
                  <div
                    key={index}
                    className={`relative p-8 rounded-2xl transition-all duration-500 group hover:-translate-y-2 ${plan.popular
                        ? 'bg-[#152540]/60 border border-[#4a9eda]/50 shadow-[0_0_30px_rgba(38,116,178,0.15)]'
                        : 'bg-[#0f1a30]/60 border border-gray-700/50 hover:border-gray-600'
                      }`}
                    style={{ backdropFilter: 'blur(20px)' }}
                  >
                    {plan.popular && (
                      <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#4a9eda] text-[#0a1525] text-xs font-bold px-4 py-1 rounded-full shadow-lg tracking-wider">
                        MOST POPULAR
                      </div>
                    )}

                    <h3 className="text-white text-xl font-bold mb-2" style={{ fontFamily: 'Geo, sans-serif', letterSpacing: '0.05em' }}>
                      {plan.name}
                    </h3>
                    <p className="text-gray-400 text-sm mb-6 h-10" style={{ fontFamily: 'Nunito, sans-serif' }}>
                      {plan.description}
                    </p>

                    <div className="flex items-baseline gap-1 mb-6">
                      <span className="text-4xl text-white font-bold" style={{ fontFamily: 'Geo, sans-serif' }}>
                        {plan.monthlyPrice === "Custom" ? "Custom" : `$${isAnnual ? plan.yearlyPrice : plan.monthlyPrice}`}
                      </span>
                      {plan.monthlyPrice !== "Custom" && (
                        <span className="text-gray-500 text-sm" style={{ fontFamily: 'Nunito, sans-serif' }}>/mo</span>
                      )}
                    </div>

                    <Link
                      to="/login"
                      className={`block w-full py-3 rounded-xl text-center text-sm font-semibold transition-all duration-300 mb-8 ${plan.popular
                          ? 'bg-gradient-to-r from-[#2674b2] to-[#4a9eda] text-white hover:shadow-[0_0_20px_rgba(38,116,178,0.4)]'
                    ...
                    <ul className="space-y-4">
                      {plan.features.map((feature, idx) => (
                        <li key={idx} className="flex items-start gap-3 text-sm text-gray-300" style={{ fontFamily: 'Nunito, sans-serif' }}>
                          <Check className="w-4 h-4 text-[#4a9eda] mt-0.5 flex-shrink-0" />
                          <span>{feature}</span>
                        </li>
                      ))}
                    </ul>
